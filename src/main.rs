...
fn main() {
    ...
    download.deactivate();
    // deactivate button so state can handle it
    ...
    while app.wait() {
        ...
        match reciever.recv() {
            Some(Message::HasLink) => { // link access state
            ...
            Some(Message::DownloadReady) => { // download press state
            ...
            Some(Message::DownloadFinished) => { // download finished state
                // empty for now
            }
            None => {} // else handler
        }
